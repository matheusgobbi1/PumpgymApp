...
interface CalendarProps {
  ...
  selectedDate: Date;
  workouts?: { [date: string]: any }; // Propriedade opcional para verificar se há treinos
}
...
export default function Calendar({
  ...
  // Verifica se uma data tem treinos registrados
  const hasRegisteredWorkouts = useCallback(
    (date: Date) => {
      const dateString = format(date, "yyyy-MM-dd");
      
      // Verifica se existem treinos para esta data
      if (!workouts[dateString]) {
        return false;
      }
      
      // Verifica se há pelo menos um tipo de treino com exercícios
      const workoutIds = Object.keys(workouts[dateString]);

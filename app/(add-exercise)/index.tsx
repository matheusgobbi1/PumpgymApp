...
const SearchResultsSkeleton = () => {
  ...
  return (
    <>
      {[...Array(5)].map((_, index) => (
        <ExerciseItemSkeleton key={`skeleton_${index}_${theme}`} index={index} />
      ))}
    </>
  );
};
  ...
  return (
    ...
    >
      <TouchableOpacity
        ...
        activeOpacity={0.7}
      >
        <View style={[styles.exerciseCardIcon, { backgroundColor: workoutColor + '20' }]}>
          <Ionicons 
            name={
        ...
        <View style={styles.exerciseDetails}>
          <View style={styles.exerciseDetailItem}>
            <Ionicons name="body-outline" size={14} color={colors.text + '80'} />
            <Text style={[styles.exerciseDetailText, { color: colors.text + '80' }]}>
              {exercise.muscle}
            </Text>
          </View>
          
          <View style={styles.exerciseDetailItem}>
            <Ionicons name="barbell-outline" size={14} color={colors.text + '80'} />
            <Text style={[styles.exerciseDetailText, { color: colors.text + '80' }]}>
              {exercise.equipment}
            </Text>
          </View>
    ...
    >
      ...
      >
        <Text style={[
          ...
          { color: selectedMuscle === null ? colors.primary : colors.text + '80' }
        ]}>
          Todos
        </Text>
      </TouchableOpacity>
      ...
      {muscleGroups.map((muscle, index) => (
        <TouchableOpacity
          ...
          style={[
            styles.muscleFilterItem,
            selectedMuscle === muscle && { backgroundColor: colors.primary + '20' }
          ]}
          onPress={() => onSelectMuscle(muscle)}
        >
          <Text style={[
            styles.muscleFilterText,
            { color: selectedMuscle === muscle ? colors.primary : colors.text + '80' }
          ]}>
            {muscle}
          </Text>
        </TouchableOpacity>
...
export default function AddExerciseScreen() {
  ...
  const handleQuickAdd = useCallback((exercise: ExerciseData) => {
  ...
  const navigateToExerciseDetails = useCallback((exercise: ExerciseData) => {
    ...
    <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      <View style={styles.filterWrapper}>
        ...
        >
          ...
          >
            <Text style={[
              styles.muscleFilterText,
              { color: selectedMuscle === null ? workoutColor : colors.text + '80' }
            ]}>
              Todos
            </Text>
          </TouchableOpacity>
          ...
          {muscleGroups.map((muscle, index) => (
            <TouchableOpacity
              ...
              style={[
                styles.muscleFilterItem,
                selectedMuscle === muscle && { backgroundColor: workoutColor + '20' }
              ]}
              onPress={() => handleSelectMuscle(muscle)}
            >
              <Text style={[
                styles.muscleFilterText,
                { color: selectedMuscle === muscle ? workoutColor : colors.text + '80' }
              ]}>
                {muscle}
              </Text>
            </TouchableOpacity>
      ...
      >
        ...
        {recentExercises.length > 0 && !searchQuery && !selectedMuscle && (
          <View key={`recent-exercises-${theme}`} style={styles.recentExercisesSection}>
